...
from sqlalchemy import Column, Integer, String, Float, DateTime, ForeignKey, func,desc, extract, ForeignKeyConstraint, create_engine
...
class Sales(Base):        #   продажи
    ...
    item_id = Column(Integer, ForeignKey(Item.id), nullable=False, comment="Идентификатор товара")
    store_id = Column(Integer, ForeignKey(Store.id), nullable=False, comment="Идентификатор магазина")
...
session.query(Store).delete()              # заполнение Store
session.commit()
...
session.query(Item).delete()                # заполнение Item
session.commit()
...
session.query(Sales).delete()                # заполнение Sales
session.commit()
a=timedelta(days=31)
